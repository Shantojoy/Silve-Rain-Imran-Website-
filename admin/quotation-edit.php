...
if($_SERVER['REQUEST_METHOD']==='POST'){
  try{
   $customerId=(int)$_POST['customer_id'];$issueDate=$_POST['issue_date'];$valid=$_POST['valid_until'] ?: null;
   $item=trim($_POST['item_name']);$qty=(float)$_POST['quantity'];$unit=(float)$_POST['unit_price'];
...
<div class="col-md-6"><label>Select Product (Auto price)</label><select id="productSelect" class="form-select"><option value="">--Manual Entry--</option><?php foreach($products as $p): ?><option value="<?= htmlspecialchars($p['name']); ?>" data-price="<?= $p['price']; ?>"><?= htmlspecialchars($p['name']); ?> ($<?= number_format($p['price'],2); ?>)</option><?php endforeach; ?></select></div>
<div class="col-md-6"><label>Item Name (manual or auto)</label><input name="item_name" id="itemName" class="form-control" placeholder="Wallpaper Installation"></div><div class="col-md-3"><label>Qty</label><input type="number" step="0.01" name="quantity" id="qty" class="form-control" value="1"></div><div class="col-md-3"><label>Unit Price</label><input type="number" step="0.01" name="unit_price" id="unitPrice" class="form-control" value="0"></div><div class="col-12"><label>Notes</label><textarea name="notes" class="form-control" rows="2"></textarea></div><div class="col-12"><button class="btn btn-dark">Save Quotation</button></div></form></div>
<script>
document.getElementById('productSelect').addEventListener('change', function(){
  const opt = this.options[this.selectedIndex];
  if(opt.value){
    document.getElementById('itemName').value = opt.value;
    document.getElementById('unitPrice').value = opt.getAttribute('data-price');
  }
});
</script>
