...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $senderAddress = sanitize($_POST['email_sender_email'] ?? '');
        ...
        $pdo->prepare('UPDATE settings SET site_name=?,site_logo=?,favicon=?,site_description=?,contact_email=?,phone=?,address=?,payment_instructions=?,whatsapp_number=?,whatsapp_message=?,email_sender_name=?,email_sender_email=? WHERE id=?')
            ->execute([$siteName,$logo,$favicon,$siteDescription,$contactEmail,$phone,$address,$paymentInstructions,$whatsappNumber,$whatsappMessage,$senderName,$senderAddress,$settings['id']]);
...
<div class="col-md-6"><label class="form-label">Favicon</label><input type="file" name="favicon" class="form-control" accept="image/png,image/x-icon"><?= helpText('Upload favicon (.ico or 32x32 PNG) shown in browser tab.'); ?></div>
...
<div class="col-md-6"><label class="form-label">Email Sender Address</label><input name="email_sender_email" class="form-control" value="<?= htmlspecialchars($settings['email_sender_email'] ?? ''); ?>"></div>
