...
if (isset($_GET['delete'])) {
    ...
    if (!empty($prod['main_image'])) {
        deleteUploadedFile(__DIR__ . '/../uploads/products/' . $prod['main_image']);
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if (!$name || !$description || $price < 0) {
            throw new RuntimeException('Valid name, price and description are required.');
        }
        ...
        if ($id) {
            ...
            $pdo->prepare('UPDATE products SET name=?,slug=?,price=?,description=?,category_id=?,main_image=? WHERE id=?')
                ->execute([$name, $slug, $price, $description, $categoryId, $mainImage ?: $old['main_image'], $id]);
            ...
            $pdo->prepare('INSERT INTO products (name,slug,price,description,category_id,main_image,is_virtual) VALUES (?,?,?,?,?,?,1)')
                ->execute([$name, $slug, $price, $description, $categoryId, $mainImage]);
...
if (isset($_GET['edit'])) {
    ...
    if ($edit) {
        $im = $pdo->prepare('SELECT * FROM product_images WHERE product_id=? ORDER BY id DESC');
        $im->execute([$edit['id']]);
        $editImages = $im->fetchAll();
    }
}
...
<div class="card p-3 shadow-sm mb-3">
    ...
    <div class="d-flex flex-wrap gap-2 mt-2">
        <?php foreach ($editImages as $img): ?>
            <div>
                <img src="../uploads/products/<?= htmlspecialchars($img['image']); ?>" width="80" alt="gallery image"><br>
                <a class="text-danger small" href="product-edit.php?id=<?= (int)$edit['id']; ?>&delete_image=<?= $img['id']; ?>&edit=<?= (int)$edit['id']; ?>">remove</a>
            </div>
        <?php endforeach; ?>
    </div>
</div>
...
<div class="card shadow-sm">
    <div class="card-body border-bottom">
        <form class="row g-2">
            <div class="col-md-4"><input class="form-control" name="search" placeholder="Search product" value="<?= htmlspecialchars($search); ?>"></div>
            <input type="hidden" name="view" value="<?= htmlspecialchars($view); ?>">
            <div class="col-md-2"><button class="btn btn-outline-dark"><i class="bi bi-search"></i> Search</button></div>
        </form>
    </div>
    <div class="table-responsive">
        <table class="table mb-0">
            <thead><tr><th>Name</th><th>Slug</th><th>Category</th><th>Price</th><th>Main Image</th><th></th></tr></thead>
            <tbody>
            <?php foreach ($rows as $r): ?>
                <tr>
                    ...
                    <td>
                        <a class="btn btn-sm btn-primary" href="product-edit.php?id=<?= $r['id']; ?>"><i class="bi bi-pencil"></i></a>
                        <a class="btn btn-sm btn-danger" data-confirm="Delete this product?" href="?delete=<?= $r['id']; ?>&view=<?= urlencode($view); ?>"><i class="bi bi-trash"></i></a>
                    </td>
                </tr>
            <?php endforeach; ?>
            </tbody>
    ...
    <div class="card-body">
        <?php for ($i = 1; $i <= $pagination['total_pages']; $i++): ?>
            <a class="btn btn-sm <?= $i === $pagination['page'] ? 'btn-dark' : 'btn-outline-dark'; ?>" href="?page=<?= $i; ?>&search=<?= urlencode($search); ?>&view=<?= urlencode($view); ?>"><?= $i; ?></a>
        <?php endfor; ?>
    </div>
</div>
<?php endif; ?>
